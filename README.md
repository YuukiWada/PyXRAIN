...
```
で使用できます。スクリプト中に表示範囲が書き込まれているので、適宜変更して使用してください。使用にはGeopandasが必要ですので、pipでインストールしてください。また都道府県境データが必要ですので、japan.geojson
```
https://github.com/dataofjapan/land/blob/master/japan.geojson
```
をダウンロードして、スクリプトと同じディレクトリに格納して使用してください。

...
本ライブラリではXバンド帯レーダの生データおよび一次補正済みデータのバイナリを扱えます。また2023年5月に合成雨量のバイナリを扱うライブラリを追加しました。各種データについては[東京大学データ統合・解析システムDIAS](https://diasjp.net/service/xrain-data/)より、申請・審査の上、ダウンロードが可能となります。
...
合成雨量のバイナリに関しては国土交通省MPレーダデータ フォーマット仕様 (合成レーダ雨量伝送用) Ver. 1.4に準拠しており、2018年4月5日以降のX/Cバンド合成雨量データにのみ対応しており、2020年2月11日まで配信されていたXバンドのみの合成雨量データの読み込みはできません。
...
提供されるデータには生データと一次処理データ、合成雨量があります。例えば一次処理データには減衰補正済みレーダー反射因子データ、偏波の反射因子差、伝播位相差変化率などがあります。本ライブラリでは仕様書の表4-7に記述された変換係数を用いた変換後のデータを出力します。例えば一次処理データの減衰補正済みレーダー反射因子データでは単位dBZの値が返されます。合成雨量は複数のレーダーの観測から雨量を算出したもので、雨量がmm/hで格納されています。データフォーマットなどはレーダーサイト毎の生データや一次処理データとは全く異なります。
...
```
とするとバイナリファイルが読み込まれます。`input_file`にはバイナリファイルのパスを与えてください。ここではインスタンス名を`radar`としておきます。読み込みが終わるとライブラリのクレジットが表示され\\
ます。バッチ処理などでクレジットを表示させたくない場合は
```
radar = xrain(input_file, False)
```
とするとクレジットは表示されません。
...
とすると雨量の格納された2次元配列が返されます。配列の範囲を取得するには
```
edge = radar.edge
```
とすると、格納範囲 (緯度・経度) が返されます。プロット用に2次元の座標配列を得るには

```
x,y = radar.mesh
```
とします。このときxとyの配列数はrainよりも1つずつ大きくなっていて、rainに格納されている各セルの境界を示しています。
```
p = ax.pcolormesh(x, y, rain)
```
とすることで簡単に合成雨量をプロットすることができます。
...
```
出力されたCSVファイルは行ごとに1セクターのデータが格納されています。セクター数300レンジ数534の場合、300行534列のCSVファイルとなっています。

PPIを出力するサンプルスクリプトは
```
python plot_ppi.py <ファイル名>
```
と実行してください。レーダーを中心とする120 km四方のエリアのPPIをプロットします。ファイルはRZH0のデータである必要があります。

RHIを出力するサンプルスクリプトは
```
python plot_rhi.py <方位角> <ファイルリスト>
```
と実行してください。方位角は北から時計回りで、最も近い方位角の観測データを探します。ファイルリストには読み込ませる複数のPPIのデータのパスが含まれたテキストファイルを読ませます。サンプルデータは`list.dat`を参照してください。XRAINでは5分間に合計12個の仰角をスキャンしますので、12個の観測データのパスを格納してください。仰角に重複があるとエラーとなります。ファイルはRZH0のデータである必要があります。

### 合成雨量
バイナリファイルを読ませると2次元の合成雨量をCSVで出力するサンプルコードを準備しました。以下のようにして使用できます。
```
python composite2csv.py /path_to_file/name_of_file /path_of_output/
```

また都道府県境とともにプロットするサンプルスクリプトは
```
python plot_composite.py /path_to_file/name_of_file
```
で使用できます。スクリプト中に表示範囲が書き込まれているので、適宜変更して使用してください。使用にはGeopandasが必要ですので、pipでインストールしてください。また都道府県境データが必要ですので、[japan.geojson](https://github.com/dataofjapan/land/blob/master/japan.geojson)をダウンロードして、スクリプトと同じディレクトリに格納して使用してください。

なおMatplotlibの仕様でエラーが出ますが、当面は無視して使用してください。また表示範囲が広い場合は描画が重くなります。
